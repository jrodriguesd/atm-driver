...
public class HsmThales implements HSM  
{
    ...
    {
    	thalesKeyTypesMap.put(KeyType.TMK,"002");
    	thalesKeyTypesMap.put(KeyType.TPK,"002");
    	thalesKeyTypesMap.put(KeyType.TAK,"003");
	...
	public String generateKey(KeyType keyType) {
		FSDMsg fsdHsmMsgOut = new FSDMsg("file:cfg/hsm/hsm-");
		String thalesKeyType = thalesKeyTypesMap.get(keyType);
		
		if (thalesKeyType == null) return null;

		fsdHsmMsgOut.set("command",    "A0");
		fsdHsmMsgOut.set("mode",       "0");
		fsdHsmMsgOut.set("key-type",   thalesKeyType);
		fsdHsmMsgOut.set("key-scheme", "U");
		...
		if (fsdHsmMsgIn != null)
		{
    		...
    		if (rc == 0)
    		{
    			String key = fsdHsmMsgIn.get("key");
    			return key;
    		}
